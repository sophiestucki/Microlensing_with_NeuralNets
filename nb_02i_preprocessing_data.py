...
def setSamplingParameters(v_t, data_dir, season_gaps, noise=False):
  """
  ...
  if v_t == 500 or v_t==0:
     if season_gaps:
         filename = data_dir + 'v%d_gaps_new/numLcurvesPerRadius_v%d_gaps.npy'%(v_t, v_t)
         total_lcurves = np.load(filename) 
         n_sample, n_sample_max, n_pix = total_lcurves, int(np.sum(total_lcurves)), 955
     else:
         filename = data_dir + 'v%d/numLcurvesPerRadius_v%d.npy'%(v_t, v_t)
         total_lcurves = np.load(filename)
         n_sample, n_sample_max, n_pix = total_lcurves, int(np.sum(total_lcurves)), 4397
         #n_sample, n_sample_max, n_pix = total_lcurves, int(np.sum(total_lcurves)), 4546
  else:
      filename = data_dir + 'v%d/numLcurvesPerRadius_v%d.npy'%(v_t, v_t)
      total_lcurves = np.load(filename)
      n_sample, n_sample_max, n_pix = total_lcurves, int(np.sum(total_lcurves)), 1137
...
def getFilename(data_dir, idx, r, v_t, sample_params, season_gaps, new_data_set):
  ...
  """
  if v_t == 300 or v_t == 500 or v_t == 0 and season_gaps == False:
  ...
  if (v_t == 500 or v_t == 0) and season_gaps == True:
    # if it is thhe new data set generated by Eric
    if new_data_set: 
...
def loadData(filename, idx, l_curves, sample_params, r, euler_sampling, new_data_set):
  ...
  for i in range(len(lc[:])):
    ...
    if r*sample_params[0][idx]+count == sample_params[1]:
      break 
