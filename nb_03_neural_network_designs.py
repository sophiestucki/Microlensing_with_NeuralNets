...
def buildCNN(num_pix, num_filter, kern_size, maxpoolsize, num_hidden_nodes, \
	dropout_ratio, r_0):
  """	
  ...
  @r_0 :: array of all the scale radii/ number of multi-classes
  ...
  # convolutional layers followed by activation function and pooling
  for i in range(len(num_filter)):

    outputs = Conv2D(filters=num_filter[i], kernel_size = [kern_size[i], 1], strides= [1,1], padding='same')(outputs)
    
    outputs = Activation('relu')(outputs)

    if maxpoolsize[i] is not None:
    	outputs = MaxPooling2D([maxpoolsize[i],1], strides=[maxpoolsize[i],1])(outputs) 
  
  # global pooling, flattening
  outputs = GlobalMaxPoolingSp2D()(outputs)
  outputs = Flatten()(outputs)

  # joining the network to the fully connected layers
...
"""### 3.3 Residual neural network: ResNet with 7-layers, and 2 residual blocks
"""

def buildResNet7(num_filter, kern_size, maxpoolsize, num_hidden_nodes,\
                           dropout_ratio, shortcut_link, batch_norm, length_traj, r_0):
	"""
	Function to build the model architecture, set optimizers and compile the model 
	@num_pix :: used to define the shape of the input layer  
	@num_filters :: arr with the ascending order of filters in the conv2D layers
	...
	for i in range(len(num_filter)):
		if shortcut_link and not execute_shortcut and shortcut_link[count_shortcut] == i:
		  out_shortcut = outputs
		  execute_shortcut = True
		  count_shortcut += 1
		  
		outputs = Conv2D(filters=num_filter[i], kernel_size = [kern_size[i], 1],\
		...
		if shortcut_link and execute_shortcut and shortcut_link[count_shortcut] == i:
	...
	# joining the fully connected network
	outputs = GlobalMaxPoolingSp2D()(outputs)
	outputs = Flatten()(outputs)
...
"""### 3.3 Residual neural network: ResNet with 18 layers 
"""

def buildResNet18(num_pix, num_filter, kern_size, n_block, \
                                 maxpoolsize, num_hidden, dropout_ratio, r_0,\
                                 batch_norm = True):
  """
  Function to build the model architecture, set optimizers and compile the model 
  @num_pix :: used to define the shape of the input layer  
  ...
  @n_block :: number of residual blocks in the ResNet
  @maxpoolsize :: arr with the pool sizes for the corresponding conv2D layers
  @num_hidden_nodes :: no. of nodes in the FNN layer right after the CNN
  @dropout_ratio :: weight constrain on the dropout layer of the FNN 
  @bath_norm :: bool decided wether or not to normalize the output from a layer
  ...
  # first shortcut layer
  out_shortcut = outputs

  for i in range(n_block) : 
      for m1 in range(len(num_filter)):
      	# convolutional layers within a residual block
        outputs = Conv2D(filters=num_filter[m1], kernel_size = [kern_size[m1], 1],\
                         strides = [1,1], padding='same')(outputs)
        outputs = Activation('relu')(outputs)
      
      # executes the identity shortcut mapping
